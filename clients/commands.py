...
def delete(ctx, client_uid):
    ...
    if client:
        client_to_deleted = _deleted_client_flow(Client(**client[0]))

        if client_to_deleted:
            client_service.delete_client(client_to_deleted)
            click.echo('Client deleted successfully')
        else:
            click.echo('Aborted')

def _deleted_client_flow(client):
    click.echo('Client with UID: {uid} was found.'.format(uid = client.uid))

    delete_confirmation = click.prompt('Delete [Y] / [N]', type = str, default='Y')
